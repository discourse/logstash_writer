...
describe LogstashWriter do
  ...
  describe "#resolve_server_name" do
    ...
    it "handles a hostname:port pair" do
      ...
      expect(mock_resolv).to receive(:getaddresses).with("logstash").and_return([Resolv::IPv4.create("192.0.2.42"), Resolv::IPv6.create("2001:108::42")])

      expect(TCPSocket).to receive(:new).with("192.0.2.42", 5151)
      expect(TCPSocket).to receive(:new).with("2001:108::42", 5151)
    ...
    it "handles a hostname:port pair with no addresses" do
      ...
      expect_log_message(mock_logger, :warn, /No addresses resolved.*logstash/)
